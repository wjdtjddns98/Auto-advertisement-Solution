...
def test_veo_fal_lite_cost_matches_unit_price():
    ...
    assert "영상 생성 (Veo(fal) Lite)" in labels

    video_item = next(i for i in cost.items if i.label.startswith("영상 생성"))
    assert abs(video_item.usd - 0.05 * 32.0) < 1e-9  # 정확히 초당 단가 × 길이
    # 합계 = 프레임(0.04) + 영상(1.60) + 텍스트 추정(작은 양수)
    assert cost.total_usd > 0.04 + 1.60
    assert cost.dry_run is True


def test_veo_fal_fast_unit_price():
    ...
    video_item = next(i for i in cost.items if i.label.startswith("영상 생성"))
    assert "Veo(fal) Fast" in video_item.label
    assert abs(video_item.usd - 0.15 * 10.0) < 1e-9


def test_veo_fal_standard_unit_price():
    ...
    cost = estimate_run_cost(_run_with_video(10.0), settings)
    video_item = next(i for i in cost.items if i.label.startswith("영상 생성"))
    assert "Veo(fal) Standard" in video_item.label
    assert abs(video_item.usd - 0.40 * 10.0) < 1e-9
...
def test_orchestrator_attaches_cost_to_run():
    ...
    # 영상 라인이 dry_run 실측 길이로 채워졌는지(8초 클립 × 비트) — veo_fal 단가 적용.
    video_item = next(i for i in run.cost.items if i.label.startswith("영상 생성"))
    assert "Veo(fal)" in video_item.label
