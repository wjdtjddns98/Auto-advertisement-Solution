...
def test_validate_script_body_rejects_shouty_hook_opener():
    ...
    def _body(hook: str) -> str:
        return "\n".join(
            [
                hook,
                "근데 진짜 문제는 따로 있어. 그 조각이 장을 막는 게 위험해.",
                "하루 열량 십 퍼센트가 상한이야. 그 선만 지키면 걱정 없다.",
            ]
        )
...
def test_script_system_prompt_pins_beat_char_range():
    """SCRIPT_SYSTEM_PROMPT가 비트당 길이 범위(30~36자)를 명시한다(리버트 가드).

    하한(30자)은 비트 사이 빈 구간을 막고, 상한(36자)은 발화가 약 7초 안에 끝나 끝
    글리치 구간을 적응 트림으로 잘라낼 여유를 남긴다(2026-06-29 PO: 8초 꽉 채우면
    잘라낼 여유가 없어 글리치가 남고, 고정 트림은 대본별로 대사가 잘림).

    ⚠️ 2026-07-31: 종전 상한 44자는 산술적으로 8초에 안 들어갔다 — Veo 실측 발화 속도가
    약 5.2자/초라 44자면 8.5초다. run26에서 평균 40.7자 대사가 꼬리 0.15초만 남기고
    잘려 PO가 "대본 끝까지 안들어감 대사 잘림"으로 편을 반려했다. 36자(≈6.9초)로 내려
    여유 1.1초를 확보한다. 회귀 방지 핀.
    """
    assert "30~36자" in SCRIPT_SYSTEM_PROMPT  # 발화 ~7초 종료(끝 여유 확보)
    assert "36자를 넘겨" in SCRIPT_SYSTEM_PROMPT  # 상한(잘림 방지)
...
def _valid_body() -> str:
    """하드룰 4줄(각 30~36자) 전부 통과하는 대본 본문.

    2026-07-31: 상한이 48→38자로 내려가 픽스처를 줄였다(대사 잘림 실측 — 원문 주석은
    `_BEAT_MAX_CHARS` 참조).
    """
    lines = [
        "강아지 간식 양 열에 아홉은 잘못 알고 있어요 지금 확인해 보세요",
        "체중 일 킬로그램당 적정 열량 기준이 있어요 간식은 십 퍼센트",
        "적정량은 고정이 아니라 활동량에 따라 조금씩 달라지니 살펴보세요",
        # 2026-07-30 PO: 마지막 비트는 유도 없이 정보로 끝낸다(계산기·링크 언급 금지).
        "몸무게 기준으로 하루 간식 양을 정해두고 그대로 지키면 돼요",
    ]
    assert all(30 <= len(x) <= 36 for x in lines), [len(x) for x in lines]
    return "\n".join(lines)
