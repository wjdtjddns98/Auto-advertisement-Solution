...
대본을 **비트(훅 → 핵심설명 → 마무리/CTA)** 단위로 나눠 비트별 독립 클립을 만든 뒤 ffmpeg로 이어 붙입니다(세로 9:16 쇼츠). Veo가 임의로 박는 깨진 자막은 프롬프트로 억제하고, 대신 비트별 대사를 ffmpeg drawtext 한글 자막으로 통제해 굽습니다(`NUTTI_CAPTION_BURN`, 무음 시청 대응). 비트 경계는 짧은 디졸브 + 교차 펀치인(`NUTTI_VEO_FAL_PUNCH_IN_SCALE`)으로 화면 크기를 교차시켜 점프컷 느낌을 줄입니다. 영상 스택은 **fal.ai 단일 결제처**로 통일돼 있습니다.
