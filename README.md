...
## 영상 백엔드 (`NUTTI_VIDEO_BACKEND=veo_fal`)

대본을 **비트(훅 → 핵심설명 → 마무리/CTA)** 단위로 나눠 비트별 독립 클립을 만든 뒤 ffmpeg로 이어 붙입니다(세로 9:16 쇼츠). 대사는 음성으로만 내보내고 Veo가 임의로 박는 깨진 자막은 프롬프트로 억제합니다(통제된 한글 자막 굽기는 `NUTTI_CAPTION_BURN=true` 옵트인 — 기본 꺼짐, 2026-07-06 PO 시사 판정). 비트 경계는 짧은 디졸브 + 교차 펀치인(`NUTTI_VEO_FAL_PUNCH_IN_SCALE`)으로 화면 크기를 교차시켜 점프컷 느낌을 줄입니다. 영상 스택은 **fal.ai 단일 결제처**로 통일돼 있습니다.

| 단계 | 소스 | 특징 |
|------|------|------|
| 시작 프레임 | **fal.ai FLUX.1 Kontext [pro]** — 마스코트 레퍼런스 이미지를 편집 | 편별 의상·장소를 유지한 채 캐릭터 일관성 확보 |
| 영상 | **fal.ai Veo 3.1**(`veo_fal`, 기본 Lite) image-to-video | Veo 네이티브 한국어 음성(별도 TTS 불요)·비트당 8초 클립. 앞뒤 침묵 트림 후 ffmpeg 스티칭 |

> 과거의 `veo`(Gemini API)·`kling`(무음+TTS) 백엔드는 2026-06 리팩토링에서 제거됐습니다 — fal.ai Veo 3.1(`veo_fal`)로 일일 쿼터 벽을 우회하면서 네이티브 한국어 음성·마스코트 일관성을 유지합니다. `FAL_KEY` 하나로 프레임·영상을 모두 처리합니다.
...
nutti/
  ...
  integrations/
    ...
    video.py                   # VideoStudio: 프롬프트 빌더·편별 스타일·dry_run·스티칭(파사드)
...
영상 백엔드는 `veo_fal`(fal.ai Veo 3.1) 단일이며 기본값입니다. 모델 화질은 `.env`의 `NUTTI_VEO_FAL_MODEL`(기본 Lite)로 조정합니다.
...
> 비트별 클립 스티칭·앞뒤 침묵 트림에는 **ffmpeg**(imageio-ffmpeg 번들)가 쓰입니다 — 시스템 설치 불요.
