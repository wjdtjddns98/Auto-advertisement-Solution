...
| `video.py` | `VideoStudio` 파사드: NanoBanana(Gemini 시작 프레임) → Veo 3.1 image-to-video 단일컷 8초. `VeoPromptBuilder`가 대사를 따옴표 인용(네이티브 음성). 완료 즉시 로컬 저장(48h 삭제 정책 대비) |
...
- `video`: 인증은 `x-goog-api-key` 헤더(Bearer 아님). 폴링 경계는 `elapsed < timeout`
  (off-by-one 금지). 에러 메시지는 상태 코드/예외 타입명만 — URL·operation id·응답
  본문 노출 금지(redaction). Veo 영상은 완료 즉시 다운로드해 로컬 저장(무음 유실 방지).
