...
def build_script_system_prompt(n_beats: int = 4) -> str:
    ...
    return (
        ...
        "기준으로 본다). "
        "마지막 비트(CTA)에서는 브랜드 이름('Nutti'·'누띠')을 절대 언급하지 않는다. 대신 "
        "'계산기'라는 단어를 반드시 넣어 간식 계산기 사용을 유도한다(프로필 링크에 있다). "
        "느낌표·외침 같은 들뜬 톤 대신 앞 비트와 같은 심드렁한 톤을 유지한 무심한 권유로 "
        "쓴다(예: '궁금하면 프로필 링크 계산기나 써보든가' 느낌 — 영상에서 마지막 비트 음성이 "
        "들뜨며 화자가 바뀌는 경향을 줄이기 위함, 끝에 느낌표를 쓰지 말 것). — "
...
def validate_script_body(body: str, n_beats: int = _BEAT_COUNT) -> list[str]:
    ...
    # 2026-07-28 PO "적어도 마지막은 계산기 사용을 유도하자" — 프롬프트 예시로는 자주
    # 빠져 하드룰로 강제. 계산기 유입이 이 채널의 유일한 전환 지표다.
    if lines and "계산기" not in lines[-1]:
        violations.append(
            "마지막 비트에 계산기 유도가 없음 — '계산기'를 언급한 무심한 권유로 다시 쓸 것"
        )
    return violations
