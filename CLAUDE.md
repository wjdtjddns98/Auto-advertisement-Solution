...
> PO(정성운/제니스)가 한 줄 지시 → 리드가 크기에 맞는 최소 비용 경로로 끝까지 수행.
> **속도 우선 정책(2026-06-11 PO 승인)**: 소~중형은 리드 직접 구현, 풀 팀은 대형·보안민감만.

## 0. 기본 동작
- **아침 일괄 지시**(작업 목록·"오늘 할 일"·"풀오토"·"하루 돌려"·`/nutti-day`) →
  `.claude/skills/nutti-day/SKILL.md` 루프 수행: 작업별 [빌드(아래 크기 라우팅)→검증→브랜치→커밋→PR→CI→머지]를
  무인 반복 후 종합 보고. 중간 질문으로 멈추지 말 것.
- **소~중형 개발**(버그픽스·롤백·옵션 추가·멀티파일 수정·소규모 연동) → **리드가 직접 구현
  + `nutti-reviewer`(sonnet) 1회 검증** — 팀 안 띄움. §1 가드레일(브랜치+PR+CI+재승인 머지) 동일 적용.
- **대형/보안민감**(신규 모듈·아키텍처 변경·인증/시크릿/신뢰불가 입력 처리) →
  `Workflow({ name: 'nutti-team', args: '<한 줄> lite' })` — **lite 기본**(리서치·보고 생략, 자동 상향 억제).
  PO가 "풀"·"꼼꼼히"라 하면 lite 제거, "opus"면 개발자 상향.
- **사소한 작업**(git status/commit/push, 한 줄 수정, 단순 질문, 설정 확인) → 직접 처리.
- 범위 애매하면 한 번만 확인, 합리적이면 추정하고 진행. (nutti-day 중엔 묻지 말고 추정+보고)
- 배경: 풀 팀은 작업당 에이전트 20+·토큰 ~2M·2시간 소요 + 5시간 사용량 한도를 빠르게 소진(한도 도달 시 전체 정지).
