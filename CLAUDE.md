...
## 0. 기본 동작
- **실질 개발**(기능 추가·멀티파일·리팩터·디버깅·연동) → `Workflow({ name: 'nutti-team', args: '<한 줄>' })`로 위임. 인라인 금지.
- **사소한 작업**(git status/commit/push, 한 줄 수정, 단순 질문, 설정 확인) → 직접 처리, 팀 안 띄움.
- 범위 애매하면 한 번만 확인, 합리적이면 추정하고 진행.
- PO 옵션: `opus`(그 작업 개발자 상향) · `lite`/"간단히"·"빠르게"(리서치·보고 생략, 리뷰 유지).
